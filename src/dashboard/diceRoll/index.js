...
const Dice = ({roll}) => {
  ...
  return <li className={`${BASE}-dice ${BASE}-${className}`}>{roll.value}</li>
}
...
const DiceRoll = () => {
  ...
  return (
    <>
      {rolling && (
        <div>
          Dices are rolling for{' '}
          <span className={`${BASE}-user`}>{rolling.user}</span>
          <p>
            Difficult: <strong>{rolling.difficult}</strong>
          </p>
          <ul className={BASE}>
